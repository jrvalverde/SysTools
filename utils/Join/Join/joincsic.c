...
#define MAX_OREC_SIZE		MAX_IREC_SIZE * 4
#define MAX_OFLD_SIZE		10000
...
int  get_record1(FILE *inf, char *r[MAX_FIELDS], char buf[MAX_IREC_SIZE]);
int  get_record2(FILE *inf, char *r[MAX_FIELDS], char buf[MAX_IREC_SIZE]);
...
FILE *if1, *if2, *of;
{
    ...
    nf1 = get_record1(if1, r1, buf1);
    if (nf1 == 0) eof1 = TRUE;
    nf2 = get_record2(if2, r2, buf2);
    if (nf2 == 0) eof2 = TRUE;
    i1 = i2 = 1;
...
*/
    ...
    empty_orecord(or);
    if (! eof1)
	while (get_record1(if1, r1, buf1) != 0) {
	    append_record(r1, or, 1, nf1);
	    save_record(of, or, nf1+nf2);
	    empty_orecord(or);
	}
    if (! eof2)
	while (get_record2(if2, r2, buf2) != 0) {
	    append_record(r2, or, nf1, nf2);
	    save_record(of, or, nf1+nf2);
	    empty_orecord(or);
	}

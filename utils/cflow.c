...
void qksort( char *, unsigned int, unsigned int, 
    int (*cmp)( char **, char ** ) );
...
/**   and sorts	them using qksort.  Then the functions are     **/
...

{
    ...
    qksort( (char *)ppsorttable, totalfuncs,	sizeof(struct func_ref_type *),
	    mycompare );
    ppelist = ppsorttable;
    ...
    char *ptoken, *plistwords[];
{
    ...
    /* XXX - JR - XXX Added next line to avoid core dumping */
    if (*plistwords == NULL) return -1;
...
/**   RETURN: none		           **/
/**			           **/
/**   DESCRIPTION: qksort quick sorts an    array of elements in   **/
/**   place.			           **/
/**			           **/

 ...
 *	Contains
 *		setup_record()
 *			read and parse config file and set up the record
 *		definition structure that will be used to do the
 *		conevrsion.
 *
 *	J. R. Valverde
 *
...
-------------------------------------------------------------------------
 *
 *  (C) José R. Valverde	(July, 27th, 2005)	
 *	<jrvalverde@acm.org>
 *
 *	José R. Valverde	(April, 19th, 1991)
 ...
 *
 * $Id: dbsetup.c,v 1.1 2005/08/03 08:02:32 root Exp $
 * $Log: dbsetup.c,v $
...
void setup_record()
{
    ...
    /* ingress up to MAXFIELDS field definitions */
    /*
     * The arbitary limit on static MAXFIELDS instead of using
     * dynamically allocated space comes from old times, when 
     * memory was scarce and realloc() was considered a heavy
     * call. One of these days I should change this XXX JR XXX
     */
    rc.fld_nmbr = 0;
...
static bool add_field()
{
	...
	while ((*p2) && (*p2 != '"')) {
		...
		p2++;
		/* Note: as we use fgets, all lines should be \n\0
		terminated. Thus any backslash should be followed
		at the very least by \n\0, then the loop will end.
		UNLESS it is the last line of the file and it has
		no \n and terminates with a \ for then fgets will
		return a string terminated as '\''\0', then the
		former increment would skip the \0 and the buffer
		would be overrun.
		It may also happen if an extraordinarily long line
		with a \ on the boundary is input.
		This test should catch these situations and avoid 
		a buffer overrun */
		if (*p2 == '\0') break;
	    }
	    size++;
...
static bool get_rc_delim()
{
	...
	/* empty line: insert a '\n' or else, EVERY line will
	   ...
	   beacuse we will read lines with fgets which conserves
	   the trailing '\n' char */

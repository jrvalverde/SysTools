 ...
 *  DBconv.c
 *
 *	Convert a line-oriented text database file structured as
 *
 ...
 *  EOR
 *
 *	to character-delimited format so as to make it processable by
 *  standard database management systems. In other words, this small
 *  tool takes as input a text file containing database records where
 *  fields are stored in separate lines preceded by an identification
 *  record tag, and produces as its output a new text file containing
 *  the same information arranging records with all fields in a single
 *  file separated by the specified character (defaults to TAB) and
 *  with special characters escaped using double quotes.
 *
 *	The final result is a conversion into a standard format that
 *  is understood and can be imported by most database management and
 *  spreadsheet systems, like Tab-delimited or CSV (comma separated
 *  value).
 *
 *	The command accepts various command line switches which allow
 *  you to specify the input and output files (defaults to stdin and
 *  stdout), the delimiter characters, a configuration file explaining
 *  the arrangement of input fields, and whether quoting should be
 *  enabled or a header line with field names be included in the
 *  output.
 *
 *  See also:
 *	included man page for dbconv (1)
 *
 *  Author:
 *	J. R. Valverde  (July, 27th, 2005)
 *		<jrvalverde@acm.org>
 *
 ...
 *
 * $Id: DBConv.c,v 1.1 2005/08/03 08:02:32 root Exp $
 * $Log: DBConv.c,v $
